...
| fill          | boolean | no       | When `true`, the column claims the available height of its flex-column parent (`flex-1`) and allows its own flex children to shrink below their content size (`min-h-0`). Use to build "viewport-bounded" subtrees where a child wants to internally scroll (e.g., chat surfaces, scrollable detail panes). Requires the ancestor chain up to `screen` to also propagate height — typically the path is `screen → column { fill: true } → ... → column { fill: true } → flex-grow child`. Defaults to `false` so existing pages where the column should grow with its content stay unchanged. |

- **React**: `ColumnComponent` -- `components/base/Column.tsx`
- **"use client"**: No
- **Renders**: `div.flex.flex-col`. Uses `containerProps`. Adds `flex-1 min-h-0` when `fill` is `true`.
...
| justify_items | string  | no       | Shared prop                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| gap           | string  | no       | Shared prop                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| fill          | boolean | no       | When `true`, the card claims its parent's available main-axis space (`flex-1`) and turns into a flex column (`flex flex-col`) with `min-h-0` and `overflow: hidden`. Use to host a child that wants to internally scroll inside the card (e.g., chat surfaces, scrollable detail panes). Requires the ancestor chain up to `screen` to also propagate height — typically each wrapping `column`/`card` on the path also has `fill: true`. Defaults to `false`. |

- **React**: `CardComponent` -- `components/base/Card.tsx`
- **"use client"**: No
- **Renders**: `div.bg-surface-card.border.border-border.rounded-lg.p-4` with shadow class. Uses `containerProps`. When `fill` is `true`, also adds `flex flex-col flex-1 min-h-0 overflow-hidden`. The `bg-surface-card` token equals the body background in light mode and is one elevation step lighter in dark mode (gives the card a subtle "lifted" appearance against the dark background).
...
| min_length          | number      | no       | Minimum character count. Validated client-side on change/blur — values shorter than `min_length` (and non-empty) are marked invalid (`data-sdui-invalid="true"`) and block submission via the same path as `pattern`. Empty values bypass the check (use `required` to forbid empty). The native `minLength` attribute is also set on the underlying `<input>`.                                                                                                                                                                                                                                                                                                         |
...
| match_field         | string      | no       | Name of another input within the same `form`. The current input is marked invalid (`data-sdui-invalid="true"`) and blocks submission unless its value equals that sibling's value. Validated on change/blur of either field: the current input subscribes to the sibling via the form-state context and re-runs validation when the sibling changes. Typical use: confirm-password / repeat-email pairs.                                                                                                                                                                                                                                                                |
| required_message    | string      | no       | Localized message shown beneath the input when `required` is set and the field is empty. If absent, no message is shown (the red border is the only feedback).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| pattern_message     | string      | no       | Localized message shown when `pattern` is set and the value does not match.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| min_length_message  | string      | no       | Localized message shown when `min_length` is set and the value is shorter. May contain `{min}`, interpolated to the threshold (e.g. `"Use at least {min} characters."`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| max_length_message  | string      | no       | Localized message shown when `max_length` is set and the value is longer. May contain `{max}`, interpolated to the threshold.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
